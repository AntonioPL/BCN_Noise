...
Nombre_Analisis = "Analisis No Supervisado LdLeLn"
...
for (k in 1:NUM_CLUSTERS){
  DataFiltrado[[paste("DIST_CLUSTER_",k,sep="")]]=((DataFiltrado[[NombreVariable1]]-NORMALIZACION_MEDIA[[NombreVariable1]])/NORMALIZACION_SD[[NombreVariable1]]-CENTROS[[NombreVariable1]][k])^2+((DataFiltrado[[NombreVariable2]]-NORMALIZACION_MEDIA[[NombreVariable2]])/NORMALIZACION_SD[[NombreVariable2]]-CENTROS[[NombreVariable2]][k])^2+((DataFiltrado[[NombreVariable3]]-NORMALIZACION_MEDIA[[NombreVariable3]])/NORMALIZACION_SD[[NombreVariable3]]-CENTROS[[NombreVariable3]][k])^2
}
...
Fechas=c("2019-01-31","2019-02-28","2019-03-31","2019-04-30","2019-05-31","2019-06-30","2019-07-31","2019-08-31","2019-09-30","2019-10-31","2019-11-30","2019-12-31","2020-01-31","2020-02-28","2020-03-31","2020-04-30","2020-05-31","2020-06-30","2020-07-31","2020-08-31","2020-09-30","2020-10-31","2020-11-30","2020-12-31")
i=1
rm(Resultados)
for (i in 1:length(Fechas)){
  ...
  DataAnalisisFecha=summarise(group_by(DataFiltrado, Codigo),
  ...
  DataAnalisisFecha=DataAnalisisFecha[,c("Codigo","Variable1","Variable2","Variable3")]
  colnames(DataAnalisisFecha)=c("Codigo",NombreVariable1,NombreVariable2,NombreVariable3)
  ...
  print(Fecha)
  distribucion=t(as.vector(table(DataAnalisisFecha$CLUSTER)))
  ...
  if (exists("Resultados")){
    ...
    Resultados=rbind(Resultados,data.frame(distribucion))
  } else{
    #Resultados=data.frame(Fecha=Fecha ,valor=distribucion)
    Resultados=data.frame(distribucion)
  }
...
ggplot(Resultados,aes(x=Fecha))+
  ...
  geom_point(aes(y=X1),color="black")+
  geom_line(aes(y=X2),color="red")+
  geom_point(aes(y=X2),color="red")+
  geom_line(aes(y=X3),color="green")+
  geom_point(aes(y=X3),color="green")+
  geom_line(aes(y=X4),color="blue")+
  geom_point(aes(y=X4),color="blue")+
  xlab("Date") + ylab("Number of Nodes")+
  labs(fill = "Cluster")+
  #theme_classic()+
