...
for (i in 1:dim(Maestro)[1]){
  ...
  for (h in 1:length(seleccionados)){
    ...
    if (inherits(df, 'try-error')){ 
      ...
      datos$Valord = datos$Valor
      datos$Valore = datos$Valor
      datos$Valorn = datos$Valor
      ...
      Agregado=summarise(group_by(datos,Fecha),
                         Ld = mean(Valord,na.rm = TRUE),
                         Le = mean(Valore,na.rm = TRUE), 
                         Ln = mean(Valorn,na.rm = TRUE),
                         LAMedia = mean(Valor,na.rm = TRUE),
                         ...
                         LAeq24 = 10*log10((1/(Total-Nulos))*sum(10^(Valor/10),na.rm = TRUE)),
                         SD = sd(Valor,na.rm = TRUE))
      Agregado$TNI= 4 * (Agregado$LAS10-Agregado$LAS90) + Agregado$LAS90-30
      Agregado$LDEN = 10 * log10((12*10^(Agregado$Ld/10)+4*10^(Agregado$Le/10)+8*10^(Agregado$Ln/10))/24)
  ...
  Estadisticas=Estadisticas[,c("Codigo","Direccion","Fecha","Ld","Le","Ln","LAMedia","LAeq24","LAS01","LAS10","LAS50","LAS90","LAS99","Total","Nulos","SD","TNI","LDEN","LNP")]
