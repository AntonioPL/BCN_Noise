...
CarpetaOutput = paste(fechaChar,"Modelos Supervisados")
Results = paste(fechaChar,"Resultados Modelado Supervisado")
...
Resultados = read.csv2(paste0("./",Results,"/",fechaChar,"_Evaluacion.csv"))
Resultados$Franja = as.character(Resultados$Franja)
Resultados$SizeTrain = as.character(Resultados$SizeTrain)
Resultados$SizeTest = as.character(Resultados$SizeTest)
...
##### 10. Bloque de Evaluación de Modelos #####
    ...
    if (exists("Resultados")){
      Resultados = rbind(Resultados,c(Franja,NombreModelo,dim(train)[1],dim(test)[1],scores[["accuracy"]],Loss = scores[["loss"]]))
    } else{
      Resultados = data.frame(Franja = Franja,NombreModelo = NombreModelo,SizeTrain =dim(train)[1] ,SizeTest=dim(test)[1], Accuracy = scores[["accuracy"]], Loss = scores[["loss"]])
    }
    write.csv2(times, paste0("./",Results,"/",fechaChar,"_Tiempos.csv"),row.names = FALSE)
    write.csv2(Resultados, paste0("./",Results,"/",fechaChar,"_Evaluacion.csv"),row.names = FALSE)
  }
